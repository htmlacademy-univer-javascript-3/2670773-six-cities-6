...
import offersReducer, {type OffersState} from '../store/offerSlice';
import {
  fetchOffers,
  fetchOffer,
  fetchNearbyOffers,
...
describe('offerSlice reducer', () => {
  ...
  //fetchOffers
  it('should handle fetchOffers.pending (set loading and clear error)', () => {
    const next = offersReducer(initialState, fetchOffers.pending('', undefined));
    expect(next.isOffersLoading).toBe(true);
    expect(next.error).toBeNull();
  });

  it('should handle fetchOffers.fulfilled (save items and stop loading)', () => {
